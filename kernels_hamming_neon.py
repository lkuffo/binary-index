...
cppyy.load_library('./include/neon_hamming.dylib')
...
def bench_standalone(
        vectors: np.ndarray,
        k: int,
        kernel=cppyy.gbl.HammingKernel.HAMMING_B128_VPOPCNTQ
) -> dict:
    queries = vectors.copy()
    bits_per_vector = vectors.shape[1] * 8
...
def main(
    ...
    standalone_kernels_cpp_128d = [
        ...
        (
            'HAMMING_B128_SERIAL_LUT',
            cppyy.gbl.hamming_b128_serial_lut,
            cppyy.gbl.HammingKernel.HAMMING_B128_SERIAL_LUT
        ),
        (
            'HAMMING_B128_VPOPCNTQ_WORDBYWORD',
            cppyy.gbl.hamming_b128_vpopcntq_wordbyword,
            cppyy.gbl.HammingKernel.HAMMING_B128_VPOPCNTQ_WORDBYWORD
        ),
        (
            'HAMMING_B128_SERIAL_BUILTINPOPCNT',
            cppyy.gbl.hamming_b128_serial_builtinpopcnt,
            cppyy.gbl.HammingKernel.HAMMING_B128_SERIAL_BUILTINPOPCNT
        ),
    ]
    standalone_kernels_cpp_1536d = [
        (
            'HAMMING_B1536_SERIAL',
            cppyy.gbl.hamming_b1536_serial,
            cppyy.gbl.HammingKernel.HAMMING_B1536_SERIAL
        ),
        (
            'HAMMING_B1536_VPOPCNTQ_WORDBYWORD',
            cppyy.gbl.hamming_b1536_vpopcntq_wordbyword,
            cppyy.gbl.HammingKernel.HAMMING_B1536_VPOPCNTQ_WORDBYWORD
        ),
    ]
    kernels_numba_128d = [
        # Baselines:
    ...
    for ndim in ndims:
        ...
        print(f"Profiling FAISS over {count:,} vectors with Hamming metric")
        stats = bench_faiss(
            vectors=vectors,
            k=k,
            threads=threads
